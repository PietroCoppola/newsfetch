...
func TestParseFeed_Edges(t *testing.T) {
	cases := []struct {
		...
		// encoding/xml applies its encoding check only to a ProcInst
		// whose target is spelled exactly "xml", so a one-character
		// change defeated the UTF-8-only rule. XML reserves the target
		// in any case, so the mixed-case spelling is rejected outright.
		{"non-UTF-8 declaration in a reserved mixed-case target rejected", `<?XML version="1.0" encoding="ISO-8859-1"?><rss version="2.0"><channel></channel></rss>`, true},
...
func TestParseFeed_TrailingContentAfterRoot(t *testing.T) {
	...
	cases := []struct {
		...
		// A directive is prolog-only Misc: XML's epilogue is
		// whitespace, comments and processing instructions, nothing else.
		{"a trailing doctype", "\n<!DOCTYPE x>\n", true},
		// Go surfaces the XML declaration as an ordinary ProcInst with
		// target "xml", but a declaration is legal only at the very start
		// of a document — a trailing one means a second document was
		// concatenated onto this one.
		{"a trailing xml declaration", "\n<?xml version=\"1.0\"?>\n", true},
		// XML reserves the exact three-letter target in ANY case, so the
		// mixed-case spellings are no more legal here than the lowercase
		// one — and they are the spellings Go's own encoding check misses.
		{"a trailing reserved XML target", "\n<?XML version=\"1.0\"?>\n", true},
		{"a trailing reserved XmL target", "\n<?XmL version=\"1.0\"?>\n", true},
		{"trailing whitespace and a comment", "\n  <!-- built by something -->\n", false},
		// The reservation is the exact three-letter name and nothing
		// longer: xml-stylesheet and xml-model are ordinary processing
		// instructions that browser-viewable feeds ship widely.
...
func TestParseFeed_PrologueBeforeRoot(t *testing.T) {
	// The root-finding loop discarded every token before the first
	// StartElement, so a proxy or CDN error page prepended to a feed body
	// parsed clean — the same contract hole [TestParseFeed_TrailingContentAfterRoot]
	// closes at the other end of the document.
	//
	// Discarding is not the fix either: XML's prolog legally allows the
	// XML declaration, comments, processing instructions, whitespace and
	// a DOCTYPE directive before the root, and real feeds ship all of them.
	const root = `<rss version="2.0"><channel><item><title>t</title><link>https://e.com/x</link></item></channel></rss>`
	cases := []struct {
		name    string
		prefix  string
		...
		{"a CDN error page prepended", "junk", true},
		{"stray text after a legal comment", "<!-- ok -->then junk", true},
		{"an end element before any root", "</oops>", true},
		// XML reserves the exact three-letter target in ANY case. Only
		// the exact lowercase spelling is the declaration; the rest are
		// reserved targets nobody may use — and they are precisely the
		// spellings encoding/xml's encoding check ignores, so accepting
		// them would leave a hole in the UTF-8-only rule.
		{"a reserved XML target", `<?XML version="1.0"?>`, true},
		{"a reserved XmL target", `<?XmL version="1.0"?>`, true},
		{"an xml declaration", `<?xml version="1.0" encoding="UTF-8"?>`, false},
		{"a comment", "<!-- generated by something -->\n", false},
		{"a doctype (legal here, unlike the epilogue)", `<!DOCTYPE rss PUBLIC "-//x//DTD//EN" "http://e.com/rss.dtd">`, false},
		{"leading whitespace", "\n\t  ", false},
		// The reservation is the exact three-letter name and nothing
		// longer: a prefix match here would reject the two processing
		// instructions browser-viewable feeds ship most often.
		{"a processing instruction", "<?xml-stylesheet href=\"feed.xsl\"?>\n", false},
		{"an xml-model processing instruction", `<?xml-model href="x"?>`, false},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			items, err := parseFeed([]byte(tc.prefix + root))
			if (err != nil) != tc.wantErr {
				t.Fatalf("parseFeed error = %v, wantErr %v", err, tc.wantErr)
			}
			if !tc.wantErr && len(items) != 1 {
...
func TestParseFeed_PrologueOrderingDeliberatelyUnchecked(t *testing.T) {
	// These three documents violate XML's prolog grammar and parse anyway.
	// That is a ruling (2026-09-01), not an oversight: [readPrologue]
	// gates on token KIND, never on order or cardinality.
	//
	// The reason is in readPrologue's doc. Every rule that would catch
	// these also rejects a stray newline before the declaration — the
	// classic WordPress/PHP blank-line defect — and a wrongly rejected
	// feed contributes nothing to its pool while showing the user a
	// warning only the publisher could act on. None of these documents
	// changes a single extracted item.
	//
	// Anyone tightening this must delete this test on purpose.
	const root = `<rss version="2.0"><channel><item><title>t</title><link>https://e.com/x</link></item></channel></rss>`
	cases := []struct {
		name   string
		prefix string
	}{
		{"an xml declaration after a comment", `<!-- c --><?xml version="1.0"?>`},
		{"two doctypes", `<!DOCTYPE rss><!DOCTYPE rss>`},
		{"an entity declaration outside any DTD", `<!ENTITY foo "bar">`},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			items, err := parseFeed([]byte(tc.prefix + root))
			if err != nil {
				t.Fatalf("parseFeed error = %v, want nil (deliberately accepted)", err)
			}
			if len(items) != 1 {
				t.Errorf("items = %d, want the root's own item still parsed", len(items))
			}
